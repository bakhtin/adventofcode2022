...
func RangeOverlap(rangeA []int, rangeB []int) bool {
	if rangeA[0] >= rangeB[0] && rangeA[len(rangeA)-1] <= rangeB[len(rangeB)-1] || // rangeA fully within rangeB
		...
		rangeA[len(rangeA)-1] >= rangeB[0] && rangeA[0] <= rangeB[0] || // rangeA overlaps with rangeB
		rangeA[0] <= rangeB[len(rangeB)-1] && rangeA[len(rangeA)-1] >= rangeB[0] { // rangeB overlaps with rangeA
		return true
	}
	return false
}
